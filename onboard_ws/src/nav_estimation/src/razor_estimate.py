...
class Estimator():
	...
	# Variables
		...
		# GAINS
		self.drive_gain = 4.0/100 # measured 4.0 meters/s when commanded full speed
		self.turn_gain = 1.5 # rad/sec
	...
	# Publishers and Subscribers
		...
		self.sub_imu = rospy.Subscriber('/razorIMU', Float32MultiArray, self.imuCallback)
	...
	def imuCallback(self, msg):
		...
		if (Ts > 0.5):
			...
			self.estimate.phi = msg.data[0];
			self.estimate.theta = msg.data[1];
			self.estimate.psi = msg.data[2];
	...
	def driveCallback(self, msg):
		...
		self.omegahat = (msg.lw - msg.rw)*self.turn_gain/2.0
