...
#    # Signal for inserted waypoint with args (float lat, float lon, int position)
    # Position is inserted position in flight path list
    wp_inserted = pyqtSignal(float, float, int)
#    wp_inserted = pyqtSignal(float, float, float, int)
    ...
    def emit_inserted(self, lat, lon, pos):
        self.wp_inserted.emit(lat, lon, pos)
