...
class PSOC():
	...
	# 	self.ser.close()
        ...
        self.sub_cmd = rospy.Subscriber('/joint_cmd', JointState, self.set_rover_cmd)
    ...
    # def ser_read(self, num):
    #     try:
    #         value = ord(self.ser.read(num).decode('string_escape'))
    #         return value
    #     except (ValueError):
    #         continue
    #         print 'Failed to read a byte correctly. Moving on.'
	    ...
	    while self.ser.inWaiting() > 0:
	        if self.ser.read(1).encode('hex') in 'e3':
	            ...
	            self.arm_feedback.q4 = ord(self.ser.read(1).decode('string_escape')) | (ord(self.ser.read(1).decode('string_escape')) << 8)
                    # self.arm_feedback.q1 = self.ser_read(1) | (self.ser_read(1) << 8)
                    # self.arm_feedback.q2 = self.ser_read(1) | (self.ser_read(1) << 8)
                    # self.arm_feedback.q3 = self.ser_read(1) | (self.ser_read(1) << 8)
                    # self.arm_feedback.q4 = self.ser_read(1) | (self.ser_read(1) << 8)
	            ...
	            self.science_data.humidity = ord(self.ser.read(1).decode('string_escape')) | (ord(self.ser.read(1).decode('string_escape')) << 8)
                    # self.science_data.temp = self.ser_read(1) | (self.ser_read(1) << 8)
                    # self.science_data.humidity = self.ser_read(1) | (self.ser_read(1) << 8)
